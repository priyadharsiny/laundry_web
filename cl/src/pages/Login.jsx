...
const Login = () => {
	...
	const loginCustomer = async (e) => {
		...
		try {
			...
			await getLoggedIn();
			navigate("/auth/user/packages"); // Update here
		} catch (err) {
		...
		<div>
			...
			<div className="text-gray-800 max-w-screen-2.5xl ml-auto max-h-88 pl-4 flex justify-between items-center overflow-hidden relative">
				<div className="font-semibold text-lg">
					<h1
						data-aos="fade-up"
						className="text-4xl sm:text-5xl font-extrabold pb-10 text-center"
					>
					...
					<form onSubmit={loginCustomer}>
						...
						>
							<label htmlFor="email" className="pb-1 text-base sm:text-lg md:text-xl">
								Email Address
							</label>
							<input
								type="email"
						...
						<div
							...
							data-aos="fade-up-left"
						>
							<label htmlFor="password" className="pb-1 text-base sm:text-lg md:text-xl">
								Password
							</label>
							<input
								type="password"
						...
						<button
							className="bg-light-blue text-white py-4 px-14 rounded-full mt-8 font-semibold text-sm sm:text-base md:text-lg"
							data-aos-delay="150"
							data-aos="fade-up-left"
							type="submit"
						>
				...
				<div className="max-h-88 hidden md:block">
