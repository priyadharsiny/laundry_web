...
const AdminUpdatePackage = () => {
	...
	const saveUpdatedPackage = async (e) => {
		...
		try {
			...
			getPackageDetails();
			setTimeout(() => navigate("/auth/admin/packages"), 2000); // Update navigation
		} catch (err) {
					...
					<div className="w-full pt-5 flex justify-center items-center">
						<div className="px-16 flex justify-center w-full">
							...
							>
								...
								<button
									type="submit"
									className="bg-light-blue text-white py-3 px-5 rounded-full flex justify-center items-center"
									disabled={buttonStatus}
								>
									{buttonStatus ? "Updating..." : "Update Package"}
								</button>
							</form>
						</div>
					</div>
